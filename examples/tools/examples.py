...
def main() -> None:
    ...
    This function:
    1. Sets up all available tools and the executor
    2. Displays available modes to the user
    3. Runs the selected example based on user input
    
    Available modes:
    1. Weather tool (requires OpenWeatherMap API key)
    2. Calculator tool
    3. Filesystem tool
    4. Chat with GPT (requires OpenAI API key)
    5. Chat with Claude (requires Anthropic API key)
    """
